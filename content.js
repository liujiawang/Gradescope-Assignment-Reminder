window.onload = () => {
  ...
  var timeRemainingSet = Set(); // a set of assignments that have time remaining ie. not passed due
  ...
  function createFile() {
    ...
    Object.entries(assignmentDueDateObject).forEach(([key, value]) => {
      if (timeRemainingSet.has(key)) {
        if (!isLaterThanToday(formatDateFromList(convertDateAndTime(value[0])))) {
